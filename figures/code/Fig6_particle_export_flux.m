...
for ws = [0.025 0.5 1 5]
    ...
    %grav_flux_meso(:,wscounter) = smooth(grav_flux_meso(:,wscounter),12);
...
subplot(3,2,1)
for ii = 1:5
    plot(time_meso+105,flux_meso_up(:,ii)./unique(diff(time_meso))./N0*100,'color',cmap(ii,:),'linewidth',1);
    hold on; grid on
    theh(ii) = line([0 0],[0 0],'color',cmap(ii,:),'linewidth',2);
end; clear ii
set(gca,'yscale','linear','xticklabel',{},'fontsize',16,'xlim',[min(time_meso)+105 max(time_meso)+105])
HH = ylabel('\langleNormalized particle flux\rangle [%/day]','fontsize',18,'Interpreter','tex');
POS = get(HH,'position');
POS(2) = 1;
set(HH,'position',POS); clear POS
set(gca,'ylim',[-1 50],'ytick',0:10:50);
text(60.75+105,45,'(a)','fontweight','b','fontsize',18)
text(87.5+105,45,'Upward flux','fontweight','b','fontsize',18,'HorizontalAlignment','right')
text(180,55,'Papa\_summer','HorizontalAlignment','center','FontWeight','b','FontSize',30)

YLIM = get(gca,'ylim');
POS = get(gca,'position');
subplot(3,2,3)
for ii = 1:5
    plot(time_meso+105,smooth(flux_meso_down(:,ii)./unique(diff(time_meso))/N0*100,8),'color',cmap(ii,:),'linewidth',2)
    hold on; grid on; axis ij
    blop = smooth(grav_flux_meso(:,ii)./N0*100,8);
    if ii == 5
    blop(29:38) = mean(blop(29:30));
    elseif ii == 4
    blop(118:204) = blop(118);
    end
    plot(time_meso+105,blop,'color',cmap(ii,:),'linestyle','--','Linewidth',2)
end; clear ii
...
set(gca,'ylim',[0 50],'ytick',10:10:50,'xlim',[min(time_meso)+105 max(time_meso)+105]);
text(87.5+105,45,'Downward flux','fontweight','b','fontsize',16,'HorizontalAlignment','right')
set(gca,'yscale','linear','ylim',[0 50],'position',POS2,'fontsize',16)
...
subplot(3,2,2)
for ii = 1:5
    plot(time_submeso,smooth(flux_submeso_up(:,ii)./unique(diff(time_submeso))./N0*100,8),'color',cmap(ii,:),'linewidth',1)
    hold on; grid on
end; clear ii
set(gca,'yscale','linear','xticklabel',{},'fontsize',16,'xlim',[min(time_submeso) max(time_submeso)])
POS = get(gca,'Position');
POS(1) = 0.48;
set(gca,'position',POS);
set(gca,'ylim',[0 50],'ytick',0:10:50,'yticklabel',{});
text(50.25,45,'(b)','fontweight','b','fontsize',18)
text(79,45,'Upward flux','fontweight','b','fontsize',18,'HorizontalAlignment','right')
text(65,55,'Papa\_winter','HorizontalAlignment','center','FontWeight','b','FontSize',30)

YLIM = get(gca,'ylim');
POS = get(gca,'position');
subplot(3,2,4)
for ii = 1:5
    plot(time_submeso,smooth(flux_submeso_down(:,ii)./unique(diff(time_submeso))./N0*100,8),'color',cmap(ii,:),'linewidth',1)
    hold on; grid on; axis ij
    blop = flux_submeso_down(:,ii)-flux_submeso_up(:,ii);
    plot(time_submeso,smooth(blop./unique(diff(time_submeso))./N0*100,8),'color',cmap(ii,:),'linewidth',2)
end; clear ii
POS2 = get(gca,'position');
POS2(2) = POS(2)-.005-POS2(4);
POS2(1) = POS(1);
set(gca,'ylim',[0 50],'ytick',10:10:50,'YTickLabel',{''},'xlim',[min(time_submeso) max(time_submeso)]);
text(79,45,'Downward flux','fontweight','b','fontsize',16,'HorizontalAlignment','right')
set(gca,'yscale','linear','ylim',[0 50],'position',POS2,'fontsize',16)
xlabel('Day of year','fontsize',18)
