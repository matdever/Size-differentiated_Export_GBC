...
tt = 25;
[~,~,wt0025s] = importw('/Volumes/home/mdever/particlespeed/doy25/w0025mday_summer.csv');
[~,~,wt1s] = importw('/Volumes/home/mdever/particlespeed/doy25/w1mday_summer.csv');
[~,~,wt5s] = importw('/Volumes/home/mdever/particlespeed/doy25/w5mday_summer.csv');

[~,~,wt0025w] = importw('/Volumes/home/mdever/particlespeed/doy25/w0025mday_winter.csv');
[~,~,wt1w] = importw('/Volumes/home/mdever/particlespeed/doy25/w1mday_winter.csv');
[~,~,wt5w] = importw('/Volumes/home/mdever/particlespeed/doy25/w5mday_winter.csv');
...
figure('position',[15 150 2119 1000])
subplot(2,2,1)
...
ylim([1e-2 100])
title('Summer','fontsize',35,'position',[1.6951e-05 18e1 0]);
text(-9.5,6e1,'(a)   Remin OFF','fontweight','b','fontsize',22)
HH = legend([h1 h2 h3],'0.025 m/day','1 m/day','5 m/day','location','southeast');
set(HH,'fontsize',16)

axes('position',[.36 .82 .15 .15])
...
set(gca,'YScale','log','fontsize',18,'xtick',[1:3],'XTickLabel',{},'yaxislocation','right')
grid on; box on
ylim([1e-2 100])
title('Relative Downward Biomass Flux','fontsize',18)
...
ylim([1e-2 100])
title('Winter','fontsize',35,'position',[1.6951e-05 18e+01 0]);
text(-143,6e1,'(b)   Remin OFF','fontweight','b','fontsize',22)

axes('position',[.8 .82 .15 .15])
...
tt = 25;
[~,~,wt0025s] = importw('/Volumes/home/mdever/particlespeed/doy25/w0025mday_remin_summer.csv');
[~,~,wt1s] = importw('/Volumes/home/mdever/particlespeed/doy25/w1mday_remin_summer.csv');
[~,~,wt5s] = importw('/Volumes/home/mdever/particlespeed/doy25/w5mday_remin_summer.csv');

[~,~,wt0025w] = importw('/Volumes/home/mdever/particlespeed/doy25/w0025mday_remin_winter.csv');
[~,~,wt1w] = importw('/Volumes/home/mdever/particlespeed/doy25/w1mday_remin_winter.csv');
[~,~,wt5w] = importw('/Volumes/home/mdever/particlespeed/doy25/w5mday_remin_winter.csv');
...
grid on; axis tight; xlim([-10 10])
ylim([1e-2 100])
text(-9.5,6e1,'(c)   Remin ON','fontweight','b','fontsize',22)

axes('position',[.36 .34 .15 .15])
bar(1,sum(N0025s(E<=0).*B0025_4.*E(E<=0))/totalF*100,'FaceColor',color0025,'EdgeColor',color0025)
hold on
bar(2,sum(N1s(E<=0).*B1_4.*E(E<=0))/totalF*100,'facecolor',color1,'edgecolor',color1);
bar(3,sum(N5s(E<=0).*B5_4.*E(E<=0))/totalF*100,'facecolor',color5,'edgecolor',color5);
set(gca,'YScale','log','fontsize',16,'xtick',[1:3],'XTickLabel',{},'yaxislocation','right')
grid on; box on
ylim([1e-2 100])
title('Relative Downward Biomass Flux','fontsize',18)
...
grid on; axis tight; xlim([-150 150])
ylim([1e-2 100])
text(-143,6e1,'(d)   Remin ON','fontweight','b','fontsize',22)

axes('position',[.8 .34 .15 .15])
bar(1,sum(N0025w(Ew<=0).*B0025_4.*Ew(Ew<=0))/totalFw*100,'FaceColor',color0025,'EdgeColor',color0025)
hold on
bar(2,sum(N1w(Ew<=0).*B1_4.*Ew(Ew<=0))/totalFw*100,'facecolor',color1,'edgecolor',color1);
bar(3,sum(N5w(Ew<=0).*B5_4.*Ew(Ew<=0))/totalFw*100,'facecolor',color5,'edgecolor',color5);
