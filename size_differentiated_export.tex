...
These results are robust to the range of sinking rates explored. If one considers a particle class with a sinking rate far exceeding the vertical advective velocity \cite<e.g., 100 m/day;>[]{Turner_2015}, then the associated biomass flux can be estimated by relying on the traditional 1-D paradigm, assuming $w_{tot}\approx w_s$. Combining this approximation with Equation \ref{eq: biomassjungeslope} shows that the slope of the biomass flux spectrum is positive for $\xi<5$, in which case very fast-sinking particles would dominate  vertical biomass fluxes. However, for $\xi>5$, the slope of the biomass flux spectrum becomes negative as well, meaning that the biomass flux is always dominated by the slow-sinking particle classes, regardless of the ocean dynamical regime. \change{While considered large, values of $\xi > 5$ remain realistic and fall within the range obtained from satellite-based estimates}{While values of $\xi > 5$ have been estimated by} \citeA{Kostadinov_2009} \add{using satellite data, in-situ estimated values of $\xi$ tend to range between 2 and 4.5}\cite{White_2015, Cram_2018}.
	...
	\begin{enumerate}
		...
		\end{enumerate}
		\item Remineralization processes \add{below the euphotic depth} logically reduce the amount of biomass flux. However, it unexpectedly enhances the role of slower-sinking particles, which are are advectively transported. The impact of remineralization \add{below the euphotic depth} is greater on faster-sinking particles since it affects both the biomass content and their sinking velocity.
	\end{enumerate}
